 ...
 *   are used.
 */
export function applyTerrainClamp(
    g: Vector3[],
    options: Pick<
        TerrainOptions,
        "maxHeight" | "minHeight" | "easing" | "stretch"
    >
) {
    let min = Infinity,
        max = -Infinity,
...
export function Edges(
    ...
    edges1: { top: boolean; bottom: boolean; left: boolean; right: boolean }
) {
    const numXSegments =
            Math.floor(distance / (options.width / options.widthSegments)) || 1,
        numYSegments =
            Math.floor(distance / (options.height / options.heightSegments)) ||
            1,
    ...
    let i, j, multiplier, k1, k2;
    const easing = easing1 || EaseInOut;
    let edges = edges1;
    ...
    for (i = 0; i < yl; i++) {
        for (j = 0; j < numXSegments; j++) {
            ...
            k1 = i * xl + j;
            k2 =
                (options.heightSegments - i) * xl + (options.widthSegments - j);
            if (edges.left) {
                g[k1].z = max(g[k1].z, (peak - g[k1].z) * multiplier + g[k1].z);
            }
            if (edges.right) {
 ...
 *   neighbors.
 */
export function applyTerrainSmooth(
    g: Vector3[],
    options: TerrainOptions,
    weight = 0
) {
    const heightmap = new Float64Array(g.length);
    for (
        let i = 0,
            xl = options.widthSegments + 1,
            yl = options.heightSegments + 1;
...
export function SmoothMedian(g: Vector3[], options: TerrainOptions) {
    const heightmap = new Float64Array(g.length),
        ...
        neighborKeys: number[] = [],
        sortByValue = (a: number, b: number) =>
            neighborValues[a] - neighborValues[b];
    for (
        let i = 0,
            xl = options.widthSegments + 1,
            yl = options.heightSegments + 1;
        ...
        for (let j = 0; j < yl; j++) {
            ...
            const halfKey = Math.floor(neighborKeys.length * 0.5);
            let median: number;
            if (neighborKeys.length % 2 === 1) {
                median = g[neighborKeys[halfKey]].z;
            } else {
                median =
                    (g[neighborKeys[halfKey - 1]].z +
                        g[neighborKeys[halfKey]].z) *
                    0.5;
            }
 ...
 */
export function SmoothConservative(
    g: Vector3[],
    options: TerrainOptions,
    multiplier: number
) {
 ...
 *   (g.length/2)^(1/4).
 */
export function applyTerrainStep(g: Vector3[], levels1?: number) {
 ...
 * Parameters are the same as those for {@link THREE.Terrain.DiamondSquare}.
 */
export function applyTurbulenceTurbulence(
    g: Vector3[],
    options: TerrainOptions
) {
    ...
    for (let i = 0, l = g.length; i < l; i++) {
        g[i].z =
            options.minHeight +
            Math.abs((g[i].z - options.minHeight) * 2 - range);
    }
}
